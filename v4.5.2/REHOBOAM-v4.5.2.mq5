...
input double ProfitCheckThrottleSeconds = 1; // Throttle every-tick profit checks (seconds, 0=every tick)
input bool UseDynamicBeta = true;   //  Use dynamic Beta (Kalman filter) or static Beta
input double KalmanProcessNoise = 9.510; // Kalman filter process noise (Q)
input double KalmanMeasurementNoise = 0.520; // Kalman filter measurement noise (R)

// Globals
double Beta = 0.0;
double KalmanBeta = 0.0;           // v4.7: Current Kalman-estimated Beta
double KalmanP = 1.0;              // v4.7: Kalman error covariance
CTrade Trade;
double EntryZ = 0.0;
...
//+------------------------------------------------------------------+
//| Update Beta using Kalman filter                                  | // v4.7
//+------------------------------------------------------------------+
void UpdateKalmanBeta(double priceA, double priceB)
  {
   ...
   LogEvent("BETA", "Updated Beta: " + StringFormat("%.4f", Beta) + " -> " + StringFormat("%.4f", KalmanBeta));
   Beta = KalmanBeta;
  }

//+------------------------------------------------------------------+
//| Recompute historical spreads with current Beta                   | // v4.7
//+------------------------------------------------------------------+
...
int OnInit()
  {
   ...
   // v4.7: Initialize Kalman Beta
   KalmanBeta = Beta;
   LogEvent("INIT", "Hedge Ratio (Beta): " + StringFormat("%.4f", Beta) +
            ", UseDynamicBeta=" + (UseDynamicBeta ? "true" : "false") +
            ", KalmanProcessNoise=" + StringFormat("%.6f", KalmanProcessNoise) +
   ...
   if(current_bar > last_bar)
     {
      ...
      // v4.7: Update Beta with Kalman filter if enabled
      if(UseDynamicBeta)
        {
