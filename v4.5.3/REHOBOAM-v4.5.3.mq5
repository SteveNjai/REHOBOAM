...
#property description "v4.5.1: For SL_Type=SL_Percent and TP_Type=TP_Multiple, check pair profit every tick to reduce slippage."
#property description "v4.5.2: Added dynamic Beta via Kalman filter, toggled by UseDynamicBeta. Updates Beta on new M1 bars."
#property description "v4.5.3: Replaced UseDynamicBeta with BetaMethod enum (STATIC_BETA, OLS, KALMAN). Added OLS method for Beta calculation."
...
enum BetaMethod
  {
   STATIC_BETA = 0, // Static Beta (on init)
   OLS = 1,         // Ordinary Least Squares (on new M1 )
   KALMAN = 2       // Kalman Filter (on new M1)
  };
...
input bool BypassCorrelationCheck = true; // Bypass correlation check
...
input double StopLossPercent = 1.0; // Stop loss % of entry equity
...
// Globals
double Beta = 0.0;
double KalmanBeta = 0.0;           // Current Kalman-estimated Beta
double KalmanP = 1.0;              // Kalman error covariance
CTrade Trade;
double EntryZ = 0.0;
...
double spreads[];
int spread_idx = 0;
datetime lastProfitCheckTime = 0;
...
void LogEvent(string category, string message, bool forceFileLog = false)
  {
   bool shouldLogToFile = EnableLogging && (forceFileLog || category == "INIT" || category == "ERROR" || category == "SIM" || category == "TRADE" || category == "BETA");
   if(shouldLogToFile)
     {
...
//+------------------------------------------------------------------+
//| Update Beta using Kalman filter                                  |
//+------------------------------------------------------------------+
void UpdateKalmanBeta(double priceA, double priceB)
  {
   if(priceB == 0)
     {
      LogEvent("ERROR", "Cannot update Kalman Beta: priceB is zero.");
      return;
     }
   // Prediction step
   double priorBeta = KalmanBeta; // Beta stays the same
   KalmanP += KalmanProcessNoise; // Add process noise to covariance
   // Update step
   double innovation = priceA - priorBeta * priceB; // Residual
   double measurementVariance = priceB * priceB + KalmanMeasurementNoise; // Variance of observation
   double kalmanGain = KalmanP * priceB / measurementVariance; // K = P * x / (x^2 + R)
   KalmanBeta = priorBeta + kalmanGain * innovation; // Update Beta
   KalmanP = (1.0 - kalmanGain * priceB) * KalmanP; // Update covariance
   // Clamp Beta to prevent instability
   if(MathAbs(KalmanBeta) < 0.1 || MathAbs(KalmanBeta) > 10.0)
     {
      LogEvent("BETA", "Kalman Beta out of bounds (" + StringFormat("%.4f", KalmanBeta) + "), reverting to previous Beta (" + StringFormat("%.4f", Beta) + ").");
      KalmanBeta = Beta; // Revert to previous valid Beta
      KalmanP = 1.0; // Reset covariance
      return;
     }
   // Update global Beta and log
   LogEvent("BETA", "Updated Beta (Kalman): " + StringFormat("%.4f", Beta) + " -> " + StringFormat("%.4f", KalmanBeta));
   Beta = KalmanBeta;
  }
...
bool CalculateBetaOLS()
  {
   ...
   if(CopyClose(SymbolA, PERIOD_M1, 0, RegressionPeriod, closesA) < RegressionPeriod ||
     ...
     {
      LogEvent("ERROR", "Insufficient historical data for OLS Beta calculation for " + SymbolA + " or " + SymbolB);
      return false;
     }
   ...
   if(varB == 0)
     {
      LogEvent("ERROR", "Variance of " + SymbolB + " is zero in OLS Beta calculation.");
      return false;
     }
   double newBeta = cov / varB;
   // Clamp Beta to prevent instability
   if(MathAbs(newBeta) < 0.1 || MathAbs(newBeta) > 10.0)
     {
      LogEvent("BETA", "OLS Beta out of bounds (" + StringFormat("%.4f", newBeta) + "), reverting to previous Beta (" + StringFormat("%.4f", Beta) + ").");
      return false;
     }
   LogEvent("BETA", "Updated Beta (OLS): " + StringFormat("%.4f", Beta) + " -> " + StringFormat("%.4f", newBeta));
   Beta = newBeta;
   return true;
  }

//+------------------------------------------------------------------+
//| Recompute historical spreads with current Beta                   |
//+------------------------------------------------------------------+
void RecomputeSpreads()
  {
   double closesA[], closesB[];
   ArraySetAsSeries(closesA, true);
   ArraySetAsSeries(closesB, true);
   if(CopyClose(SymbolA, PERIOD_M1, 0, LookbackPeriod, closesA) >= LookbackPeriod &&
      CopyClose(SymbolB, PERIOD_M1, 0, LookbackPeriod, closesB) >= LookbackPeriod)
     {
      ...
      LogEvent("BETA", "Recomputed " + IntegerToString(LookbackPeriod) + " historical spreads with Beta=" + StringFormat("%.4f", Beta));
     }
   else
     {
      LogEvent("ERROR", "Failed to recompute historical spreads for dynamic Beta.");
      spread_idx = 0; // Reset to build prospectively
     }
   ArrayFree(closesA);
   ArrayFree(closesB);
  }
...
int OnInit()
  {
   ...
   // Initialize Kalman Beta if using Kalman method
   if(BetaCalculationMethod == KALMAN)
     {
      KalmanBeta = Beta;
     }
   string betaMethodStr = EnumToString(BetaCalculationMethod);
   LogEvent("INIT", "Hedge Ratio (Beta): " + StringFormat("%.4f", Beta) +
            ", BetaCalculationMethod=" + betaMethodStr +
            (BetaCalculationMethod == KALMAN ? ", KalmanProcessNoise=" + StringFormat("%.6f", KalmanProcessNoise) +
                                               ", KalmanMeasurementNoise=" + StringFormat("%.6f", KalmanMeasurementNoise) : ""));
...
//+------------------------------------------------------------------+
void CheckForClosure(int direction, double zScore, double currentProfit)
  {
...
void OnTick()
  {
   ...
   // Every-tick profit check for SL_Percent mode (if position open)
   if(SL_Type == SL_Percent && TP_Type == TP_Multiple)
     {
      if(current_time >= lastProfitCheckTime + ProfitCheckThrottleSeconds)
        {
         int direction = GetPositionDirection();
         if(direction != 0)
           {
            double currentProfit = CalculatePairProfit();
            CheckForClosure(direction, 0.0, currentProfit); // zScore not needed for percent mode
            lastProfitCheckTime = current_time;
           }
        }
     }
   ...
   if(current_bar > last_bar)
     {
      ...
      // Update Beta based on selected method
      if(BetaCalculationMethod == KALMAN)
        {
         double priceA = (askA + bidA) / 2.0;
         double priceB = (askB + bidB) / 2.0;
         UpdateKalmanBeta(priceA, priceB);
         // Recompute historical spreads with new Beta
         RecomputeSpreads();
        }
      else if(BetaCalculationMethod == OLS)
        {
         if(CalculateBetaOLS())
           {
            // Recompute historical spreads with new Beta
           ...
           {
            LogEvent("ERROR", "Failed to update Beta with OLS; retaining previous Beta.");
           }
        }
      // STATIC_BETA: No update needed, Beta remains as initialized
      ...
      if(spread_idx >= LookbackPeriod)
        {
         ...
         LogEvent("TRADE", "Current Z-Score (M1): " + StringFormat("%.4f", zScore) +
                  ", BetaMethod=" + EnumToString(BetaCalculationMethod), true);
         ...
         if(direction != 0)
           {
            double currentProfit = CalculatePairProfit();
            CheckForClosure(direction, zScore, currentProfit);
           }
...
//+------------------------------------------------------------------+
//| Calculate hedge ratio (Beta) for STATIC_BETA                     |
//+------------------------------------------------------------------+
bool CalculateHedgeRatio()
  {
   ...
   Beta = cov / varB;
   LogEvent("INIT", "Calculated Beta (Static): " + StringFormat("%.4f", Beta));
   return true;
  }
