...
input int LookbackPeriod = 185;    // Lookback for mean and std dev of spread (M1 bars)
input int RegressionPeriod = 340;  // Lookback for hedge ratio calculation (M1 bars)
input double DefaultEntryZScore = 3.6; // Default entry threshold for |Z-Score|
input double MinEntryZScore = 1.6;  // Minimum Z-score threshold for trade entry
...
input double SimIntervalMinutes = 133.0; // Simulation interval (minutes)
   ...
   if(current_bar > last_bar)
     {
      ...
      // Update spread array
      double current_spread = (last_askA + last_bidA) / 2.0 - Beta * (last_askB + last_bidB) / 2.0;
         ...
         if(direction == 0)
           {
            if(MathAbs(zScore) < MinEntryZScore)
            ...
            else if(zScore <= -EntryZScore)
              {
               ...
               LogEvent("TRADE", "Z-Score (M1): " + StringFormat("%.4f", zScore), true);
