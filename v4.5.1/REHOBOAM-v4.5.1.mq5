...
#property description "v4.5.1: For SL_Type=SL_Percent and TP_Type=TP_Multiple, check pair profit every tick (not just new M1 bars) to reduce slippage on closures."
...
input double ProfitCheckThrottleSeconds = 0.0; // v4.6: Throttle every-tick profit checks (seconds, 0=every tick, e.g., 1.0=once per second)
// Globals
double Beta = 0.0;
CTrade Trade;
double EntryZ = 0.0;
...
double spreads[];
int spread_idx = 0;
datetime lastProfitCheckTime = 0; // v4.6: For throttling profit checks if needed
//+------------------------------------------------------------------+
//| Log event to file                                               |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Check for position closure (extracted for reuse)                 | // v4.6: Extracted to function for every-tick and new-bar calls
//+------------------------------------------------------------------+
...
// v4.6: Every-tick profit check for SL_Percent mode (if position open)
...
{
double currentProfit = CalculatePairProfit();
CheckForClosure(direction, zScore, currentProfit); // v4.6: Use extracted function (handles ZScore if needed)
}
