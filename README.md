...
| `Timeframe`             | ENUM\_TIMEFRAMES | PERIOD\_D1 | Chart timeframe (Daily recommended).                              |
...
| `StopZScore`            | double      | 4.0     | Z-Score threshold for stop-loss (if `SL\_Type = SL\_ZScore`).         |

| `RiskPercent`           | double      | 1.0     | % of account balance to risk per trade (if `SL\_Type = SL\_ZScore`).  |
...
| `SL\_Type`               | StopLossType| SL\_ZScore | Stop-loss type: 0 (Z-Score) or 1 (Percentage).                    |

| `StopLossPercent`       | double      | 2.0     | % of entry equity for stop-loss (if `SL\_Type = SL\_Percent`).        |

| `ExpectedAdverseSpread` | double      | 0.01    | Expected spread move for sizing (if `SL\_Type = SL\_Percent`).        |
