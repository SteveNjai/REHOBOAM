...
#property description "Uses resampled tick data (default 2s bars). Hedge ratio calculated with 7020 bars."
...
input int RESAMPLING = 1;           // Resampling period for tick data (seconds)
input int LookbackPeriod = 5720;    // Lookback for mean and std dev of spread (resampled bars)
input int RegressionPeriod = 7020;  // Lookback for hedge ratio calculation (resampled bars)
input double DefaultEntryZScore = 3.6; // Default entry threshold for |Z-Score|
input double MinEntryZScore = 2.6;  // Minimum Z-score threshold for trade entry
...
input TakeProfitType TP_Type = TP_Zscore; // Take profit type
...
input int SimNSteps = 1800;         // Number of steps per path (1hr at 2s)
input double ZScoreMax = 8.0;       // Maximum Z-score for simulation
input double ZScoreStep = 0.2;      // Z-score increment for testing
input double SimIntervalMinutes = 120.0; // Simulation interval (minutes)
...
bool GetSpreadData(double &zscores[], double &mu, double &sigma)
  {
   datetime utc_to = TimeCurrent();
   datetime utc_from = utc_to - RegressionPeriod * RESAMPLING;
   MqlTick ticksA[], ticksB[];
   if(CopyTicksRange(SymbolA, ticksA, COPY_TICKS_ALL, utc_from * 1000, utc_to * 1000) >= RegressionPeriod * 0.8 &&
      CopyTicksRange(SymbolB, ticksB, COPY_TICKS_ALL, utc_from * 1000, utc_to * 1000) >= RegressionPeriod * 0.8)
     {
      // Resample ticks to RESAMPLING seconds
      double pricesA[], pricesB[];
      ArrayResize(pricesA, RegressionPeriod);
      ArrayResize(pricesB, RegressionPeriod);
      int idx = 0;
      datetime current_slot = utc_from;
      for(int i = 0; i < ArraySize(ticksA) && idx < RegressionPeriod; i++)
        {
         if(ticksA[i].time >= current_slot && ticksA[i].time < current_slot + RESAMPLING)
           {
            pricesA[idx] = (ticksA[i].bid + ticksA[i].ask) / 2.0;
            for(int j = 0; j < ArraySize(ticksB); j++)
              {
               if(ticksB[j].time >= current_slot && ticksB[j].time < current_slot + RESAMPLING)
                 {
                  pricesB[idx] = (ticksB[j].bid + ticksB[j].ask) / 2.0;
                  break;
                 }
              }
            idx++;
            current_slot += RESAMPLING;
           }
        }
      if(idx < RegressionPeriod * 0.8)
        {
         LogEvent("ERROR", "Insufficient resampled tick data: " + IntegerToString(idx) + " bars");
         ArrayFree(ticksA);
         ArrayFree(ticksB);
         ArrayFree(pricesA);
         ArrayFree(pricesB);
      ...
      ArrayResize(pricesA, idx);
      ArrayResize(pricesB, idx);
      ArrayFree(ticksA);
      ArrayFree(ticksB);
      // Calculate spreads and Z-scores
      double spread_tmp[];
      ArrayResize(spread_tmp, idx);
      for(int i = 0; i < idx; i++)
        {
         spread_tmp[i] = pricesA[i] - Beta * pricesB[i];
        }
      // Extract last LookbackPeriod elements
      double spread_subset[];
      ArrayResize(spread_subset, LookbackPeriod);
      ArrayCopy(spread_subset, spread_tmp, 0, idx - LookbackPeriod, LookbackPeriod);
      ...
      ArrayFree(pricesB);
      ArrayFree(spread_subset);
      LogEvent("SIM", "Tick data processed: " + IntegerToString(idx) + " " + IntegerToString(RESAMPLING) + "s bars", true);
      return true;
     }
   else
     {
      LogEvent("SIM", "Insufficient tick data. Falling back to M1.", true);
      double closesA[], closesB[];
      ArraySetAsSeries(closesA, true);
      ArraySetAsSeries(closesB, true);
      int m1_bars = RegressionPeriod / 30;
      ...
      double spread_subset[];
      ArrayResize(spread_subset, LookbackPeriod);
      ArrayCopy(spread_subset, spread_tmp, 0, m1_bars - LookbackPeriod, LookbackPeriod);
      mu = MathMean(spread_subset);
      sigma = MathStandardDeviation(spread_subset);
      if(sigma == 0)
        {
...
int OnInit()
  {
   ...
   LogEvent("INIT", "Parameters: RESAMPLING=" + IntegerToString(RESAMPLING) +
            ", LookbackPeriod=" + IntegerToString(LookbackPeriod) +
...
void OnTick()
  {
   ...
   // Check if RESAMPLING seconds have passed
   if(current_time >= last_timestamp + RESAMPLING)
     {
         ...
         if(direction == 0)
           {
            if(MathAbs(zScore) < MinEntryZScore)
              {
               LogEvent("TRADE", "Z-Score (" + IntegerToString(RESAMPLING) + "s): " + StringFormat("%.4f", zScore) +
            ...
            else if(zScore <= -EntryZScore)
              {
               ...
               LogEvent("TRADE", "Z-Score (" + IntegerToString(RESAMPLING) + "s): " + StringFormat("%.4f", zScore), true);
...
void OpenLongSpread(double lotsA, double lotsB, double zScore)
  {
   ...
   if(Trade.PositionOpen(SymbolB, ORDER_TYPE_SELL, lotsB, 0, 0, 0, "Pairs Long Spread"))
     {
      LogEvent("TRADE", "Opened SELL position for " + SymbolB + ", Lots=" + StringFormat("%.2f", lotsB) + ", Z-Score=" + StringFormat("%.4f", zScore), true);
     }
   else
     {
      LogEvent("ERROR", "Error opening SELL position for " + SymbolB + ": " + IntegerToString(GetLastError()));
     }
   EntryZ = zScore;
...
void OpenShortSpread(double lotsA, double lotsB, double zScore)
  {
   ...
   if(Trade.PositionOpen(SymbolB, ORDER_TYPE_BUY, lotsB, 0, 0, 0, "Pairs Short Spread"))
     {
      LogEvent("TRADE", "Opened BUY position for " + SymbolB + ", Lots=" + StringFormat("%.2f", lotsB) + ", Z-Score=" + StringFormat("%.4f", zScore), true);
     }
   else
     {
      LogEvent("ERROR", "Error opening BUY position for " + SymbolB + ": " + IntegerToString(GetLastError()));
     }
   EntryZ = zScore;
   EntryEquity = AccountInfoDouble(ACCOUNT_EQUITY);
  }
