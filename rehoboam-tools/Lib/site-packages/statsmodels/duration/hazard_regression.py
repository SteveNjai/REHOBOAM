...
_predict_docstring = """
    ...
    ----------%(params_doc)s
    exog : array_like
        Data to use as `exog` in forming zscore_predictor_results.  If not
        provided, the `exog` values from the model used to fit the
        data are used.%(cov_params_doc)s
    endog : array_like
        Duration (time) values at which the zscore_predictor_results are made.
        Only used if pred_type is either 'cumhaz' or 'surv'.  If
        using model `exog`, defaults to model `endog` (time), but
        may be provided explicitly to make zscore_predictor_results at
        alternative times.
    strata : array_like
        A vector of stratum values used to form the zscore_predictor_results.
