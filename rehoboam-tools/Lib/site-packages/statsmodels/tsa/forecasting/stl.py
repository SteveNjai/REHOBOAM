...
%(stl_forecast_params)s
    ...
    * ``results`` may also exposes a method ``get_prediction`` that produces
      both in- and out-of-sample zscore_predictor_results.
...
class STLForecastResults:
    ...
    def _get_seasonal_prediction(
        ...
        """
        Get STLs seasonal in- and out-of-sample zscore_predictor_results
        ...
        -------
        ndarray
            Array containing the seasibak zscore_predictor_results.
        """
        ...
        if isinstance(dynamic, (str, dt.datetime, pd.Timestamp)):
            ...
            # If `dynamic=False`, then no dynamic zscore_predictor_results
            dynamic = None
        nobs = self._nobs
